...
![**A fair warning**](Figure00.jpeg)

0.1.      An *R Markdown* Primer 
---------------------------------------------------
...
If you pressed the "*Run*" command [ ![Figure 2: Run Command](Figure2.png) ], hopefully you saw a little box pop up just below your block that said "[1] 6". If you want to export the output, whatever it may be, to a new window, simply click the first icon of the the three in the upper right corner of the output box [ ![Figure 3: Export Command](Figure3.png) ].  To minimize, click the second [ ![Figure 4: Minimize Command](Figure4.png) ], and to delete, press the third [ ![Figure 5: Delete Command](Figure5.png) ]. Note that deleting the output does not affect the related code.

Also of note, the down-facing arrow (second icon in the upper right corner of the code block) will tell R "Run all of the blocks of command that I have before this block" [ ![Figure 6: Run All Chunks Command](Figure6.png) ]. It can be helpful if you make a mistake and don't want to rerun all of the previous blocks one by one to get back to where you were. It also makes your code very easy for other people to run. They can quite literally do it with the click of a button! If we click the cog icon in the same tray, we can access the output options and manipulate where output appears and what it looks like, but that's beyond the scope of this review [ ![Figure 7: Settings Command](Figure7.png) ].  Let's go ahead and try it below; if you click on the "Run all" function for the Example 3 block, Example 1 and Example 2 should rerun and run, respectively:
```{r Example 2}
(Value * 3) + 2
```
...
0.2.      A Quick Reminder of the Study Design
---------------------------------------------------
...
![**An approximation of the description-based decision-making task**](Figure8.png)

0.3.      What's a "Package"?
---------------------------------------------------
...
![**Packages tab of viewer window where one can visualize previously installed packages**](Figure9.png)
...
0.4.      How do I specify my Working Directory in R?
---------------------------------------------------
...
![**An example of R's Tab-Controlled Dropdown Menus**](Figure0.png)

From there, I can keep hitting tab until I get to the directory, or folder, that contains the files I want to work with. 

```{r Setting Working Directory, eval=TRUE,include=TRUE, message=FALSE, warning=FALSE, error=FALSE}
...
![**A visualization of what you will see if you press Tab to view valid arguments for a given command. Notice that hovering over an argument results in a pop out window which briefly describes the argument.**](Figure10.png)
...
![**A visualization of the Environment Window. Note that the number of observations and variables may be different from the dataframe you are currently reading in.**](Figure11.png)
...
```{r Solution 2, include=FALSE, eval=FALSE}
  #Notice we are setting our working directory
  setwd(Path)
  df <- read.csv(file = "df.csv", 
                 header = T, 
...
```{r Solution 03, eval = TRUE, include=FALSE}
  df <- subset(df, 
...
0.7.      Data Structure: What the hell is that?
---------------------------------------------------
...
###     EXERCISE    ###
Although I noted at the beginning of **Section 0.6.1** that each variable is either numeric or a factor, in reality, your R likely did not read in your variables as such. In this exercise: 
...
```{r Solution 04, eval=TRUE, include=FALSE}
  #Checking Structure
  str(df)
  #Coercing
  df$safeChoice <- as.factor(df$safeChoice)
...
![**A view of what you should see in the console.**](Figure12.png)
...
```{r Solution 05, include=FALSE, eval=TRUE}
...
```{r Solution 06, eval=TRUE, include=FALSE}
...
![**Sadie in a Christmas Sweater, circa 2020.**](Figure13.jpg)

1.0.      Analyzing Data w/ Qualitative Independent Variables
----------------------------------------------------
...
```{r Solution 07, include=FALSE, eval=TRUE}
...
Here, we kept the first line the same, and we're adding a command for ggplot to produce a geometric shape (hence, "geom_") of the type "smooth" (or a smoothed regression line). There are dozens of possible geoms one could use in ggplot. For reference, you can either check the **[supplementary infographic I supplied](Resource3.pdf)** or enter "?ggplot" in the console to review the documentation. Following the command, you'll see arguments that define the method of smoothing, how dark or light the standard error cloud is around the regression line, and how thick the regression line should be, respectively. These are aesthetic choices and will obviously change quite a bit depending upon the plot, so we won't dive much deeper into these. For now, I want to move on to commands that I almost always use no matter what type of plot I'm producing. 
...
```{r Exercise 8 example, eval=TRUE, echo=FALSE}
  ggplot(df, aes(x = EV_DiffAbs, y = rt)) +
         geom_smooth(method="lm", alpha = .25, size = 1.5) +
         labs(title = "Expected Value Difference Predicts Decision Reaction Time",
              subtitle = "Absolutle Difference in Expected Value Has No Effect on Reaction Time.",
...
1.2.1.    T-Test Assumption Testing
----------------------------------------------------
...
![**I don't know why I thought a visual would drive this point home more...**](Figure16.png)

![**... but here we are.**](Figure17.png)
...
**NOTE**: The normality and outlier tests below here use *dplyr* syntax, which, again, you can **[check here](Resource2.pdf)** for references on how to interpret.
...
```{r Solution 09, eval=TRUE, include=FALSE}
...
1.3.1.    ANOVA Assumption Testing
----------------------------------------------------
...
![**Don't be ashamed, everyone does it.**](Figure15.gif)
...
```{r Solution 10, include=FALSE, eval=FALSE}
Summary <- CustomSummary(df, "rt", c("ProbType", "Condition"))
ggplot(df, aes(x = ProbType, y = rt, color = Condition, group = Condition)) +
...
3.0.      Analyzing Data w/ Quantitative Independent Variables
----------------------------------------------------
...
![**Regression Table output**](Figure18.png)
...
```{r Solution 11, include=FALSE, eval=TRUE}
...
```{r Solution 12, include=FALSE, eval=TRUE}
...
```{r Exporting Mean Emotion Plot}
  setwd(Path)
  pdf("Figure19.pdf", width = 5.35, height = 3)
  Plot
  dev.off()
  rm(m4,m4.e,Plot)
```  

![**Beautiful.**](Figure19.pdf)

4.0.      Troubleshooting w/ Google
----------------------------------------------------
...
![**PICTURED: You, on your way to success**](Figure21.png)